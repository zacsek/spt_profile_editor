...
RSpec.describe SptProfileEditor::Inventory do
  ...
  let(:mock_db) do
    instance_double(SptProfileEditor::ServerDatabase,
                    items: {
                      "5449016a4bdc2d6f028b456f" => { "_parent" => "543be5dd4bdc2d3c308b4569", "_props" => { "StackMaxSize" => 500_000 } }, # Roubles
  ...
  describe "Enumerable behavior" do
    it "responds to enumerable methods" do
      ...
      expect(inventory).to respond_to(:map)
      expect(inventory).to respond_to(:select)
    end

    it "iterates over items" do
      expect(inventory.count).to eq(inventory.all_items.count)
      expect(inventory.any?(&:is_money?)).to be true
    end
  end

  describe "#delete_item" do
    it "deletes an item by id" do
      item_to_delete = inventory.all_items.first
      expect(inventory.delete_item(item_to_delete.id)).to be true
      expect(inventory.all_items.find { |i| i.id == item_to_delete.id }).to be_nil
    end

    it "returns false if item not found" do
      expect(inventory.delete_item("non_existent_id")).to be false
    end
  end

  describe "#inspect" do
    it "returns a formatted string" do
      expect(inventory.inspect).to include("SptProfileEditor::Inventory")
      expect(inventory.inspect).to include("items_count=")
      expect(inventory.inspect).to include("stash_id=")
    end
  end
end
